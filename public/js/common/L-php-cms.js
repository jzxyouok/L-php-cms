...
app.factory('categoryManageService', ['$http', function ($http) {
    return {
        get: function (title, from, display, tags, img, parent, keywords, description, type, view, author, content) {
            $http({
                method: 'POST',
...
app.controller('userGroupManageCtrl', ['$scope', '$http', 'userGroupManageService',  function ($scope, $http, userGroupManageService) {
    $scope.statusOptions = [
        {name:'启用',id:1},
        {name:'禁用',id:0},
    ];
    ...
    $scope.addUserGroup=function () {
        ...
        userGroupManageService.addAdminUserGroup(group_id,name,pid,status,remark).then(function(res) {
            if(res.data.code===1){
                $scope.msg=res.data.msg;

                $('#user_group_manage_add_modal').modal('hide');
            }else if(res.data.code===0){
                $scope.msg=res.data.msg;
                $('#user_group_manage_add_modal').modal({
                    keyboard: true
                });
            }
    ...
    $scope.powerCommit = function () {
        ...
        userGroupManageService.powerCommit($scope.group.name, zTreeObj).then(function success(res) {
            if (res.data.code === 1) {
                $scope.getUserGroup();
    ...
    var zNodes = [
        ...
        {
            id: 'user_manage_d', pId: 0, name: "用户管理", open: true,
            children: [

                {id: 'user_group_manage', pId: 'user_manage_d', name: "用户组管理"},
                {id: 'user_manage', pId: 'user_manage_d', name: "用户管理"},
     ...
     * */
    $scope.forbidden = function (name) {
        userGroupManageService.forbidden(name).then(function success() {
            $scope.getUserGroup();
        }, function error() {
     ...
     * */
    $scope.startUsing = function (name) {
        userGroupManageService.startUsing(name).then(function success() {
            $scope.getUserGroup();
        }, function error() {
    ...
    $scope.edit = function (user) {
        ...
        $scope.pid = $scope.pidOptions[Number($scope.user.pid)].id;//设置默认值
    };
    /*
     * 编辑提交
     * */
